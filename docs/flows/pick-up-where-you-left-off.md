...
- Your carried queue **survives the repo being moved or re-cloned** — scoping keys on repo identity (root-commit SHA), not the absolute path (R63).
...
- [E] `resume survives a repo MOVE — scoping keys on repo identity` ✅
...
- 2026-07-20 — resume now keys on repo identity, so a moved/re-cloned repo keeps its queue (R63).
