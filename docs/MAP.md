...
| `bin/tq-ask-guard.sh` | PreToolUse[AskUserQuestion]: while autopilot is on, hard-block the question (deny) and tell the model to decide-if-reversible or PARK as `❓` — makes autopilot's "never ask" mechanical, not advisory. |
...
| `bin/tq-design-guard.sh` | PreToolUse[Edit\|Write\|NotebookEdit]: on a visual/design prompt, deny edits until a wireframe preview (AskUserQuestion) has been shown — the ask-guard clears the per-session marker capture armed. During an autopilot drain (owner absent) it can't preview — asking is blocked too — so it PARKS the change as a `❓ [parked]` decision (denies the edit, holds the return-review gate) instead of standing down. Enforces show-before-build. `CLAUDE_TQ_DESIGN_GATE=0` disables. |
...
| `bin/tidy-verify.sh` | Stop: post-work debt surface + opt-in test gates (the end-of-turn verification floor was removed — tests are run manually). On a dirty tree, one non-blocking surface: **import cycles** touching the change (`lib/arch.sh`, content-deduped) + the throttled deliberate-prune nudge (over `CLAUDE_TIDY_PRUNE_THRESHOLD` over-budget files → `tidy-distill.sh`'s weight report, once per debt episode). Plus two OPT-IN, off-by-default test-existence gates that block until a changed file is characterized: the coverage ratchet (`CLAUDE_TIDY_COVERAGE_RATCHET=1`) and the narrow **regression gate** (block when a changed file is both a scar-tissue hotspot and untested — `CLAUDE_TIDY_REGRESSION_GATE=1`). Disable the whole hook with `CLAUDE_TIDY_CHECKS=0`. |
...
| File | Responsibility |
|---|---|
| `bin/hud-status.sh` | The status-line renderer, three zones: **health & alerts** (beacon · 🎨 design / 🔒 review edit-gates, tagged with a word only while armed · **❓N** parked decisions · **⏳N** owner-blocked) · **feature state** (🛡 safety shield / 🛡✗N disabled-floor count · bare ✈️ autopilot / 🤖 agents, shown only when on) · **context** (model · ⇡/⇣ token throughput · **project name** · branch+dirty · **↑ahead ↓behind** vs upstream). |
