...
| `flow.sh` | Render the colored workflow diagram in the terminal (`./flow.sh` / `make flow`) — the one sanctioned human-facing artifact; hand-maintained, so keep it in sync when the lifecycle changes. |
