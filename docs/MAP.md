...
| `commands/cover.md` | `/companion:cover` (R58) — rank UX paths by criticality × coverage gap and **recommend the ideal test** per critical path in the repo's own idiom, queued. **Recommends, never writes.** The test-recommendation arm of the living contract. |
...
| `bin/contract-drift.sh` | The living-contract **backstop** (R58) — generic (R9) detector that warns when behaviour changed without a contract doc (`docs/UX.md`·`NFR.md`·`INVARIANTS.md`) moving. Advisory (`exit 0`, never blocks); run by `check.sh` + `ship-it`. Prevention is the STEERING reflex; this is the net. |
| `bin/statusline.sh` | The status line (a `statusLine` command, not a hook), grouped by plugin-relevance (R34) with `:` dim section dividers: ⠋ beacon · `v<version>` (from plugin.json) · `:` 🛡 gate · ✈️ autopilot (✈️⚡ decisive, R59) · 📦 ship-mode `:` (active features) · `:` 📋 open · ❓ parked · ⏳ blocked `:` (the queue, own section) · model · ⇡in ⇣out · project · branch (+ ↑ahead ↓behind). Read-only; beacon animates at `refreshInterval:3`. Shield 🛡️ carries VS16 for emoji-width parity with ✈️/📦 (even spacing). Wire with `/companion:setup`. |
