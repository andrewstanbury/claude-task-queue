...
| `bin/resume.sh` | Manual resume — turn autopilot off (announced) + list this repo's open tasks from earlier sessions on demand (the SessionStart twin). Invoked by `/companion:resume`'s step 1 (R39, folded into review 2026-07-17); no longer its own command. |
...
| `commands/resume.md` | `/companion:resume` (R38/R39, renamed from `review` 2026-07-18) — **step 1 re-surfaces earlier-session tasks** (runs `resume.sh`, reinstates class-preserving), then walk the `❓ [parked]` + `⏳ [blocked]` pile one at a time, recommendation-first (reusing the advise loop, R29), and write picks back to `tq` before new work. **Runs automatically when autopilot is turned off.** Parked+blocked only; defer/bail allowed; no-op when empty. |
