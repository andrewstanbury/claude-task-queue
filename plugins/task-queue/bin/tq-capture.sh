...
if [ "$consequential" -eq 1 ]; then
  ...
  [ "$design" -eq 1 ] && ctx="$ctx It also changes the UI: present the proposed design as FAITHFUL ASCII mockups in the AskUserQuestion preview (recommended option first, arrow-keys + Enter to pick) so the owner can see it before you build."
