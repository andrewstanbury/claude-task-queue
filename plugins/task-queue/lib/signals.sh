...
tq_state_signals() {
  ...
  if tq_is_away "$root"; then
    block="🚶 AWAY mode is ON — the owner is away from the keyboard, so do NOT block on them: never call AskUserQuestion and never ask them to run a test or take a manual step. Keep going in auto and finish as much as you safely can, VERIFYING your own work YOURSELF — you have a shell, so a 'playtest' or manual check is something you RUN, not a reason to stop (run the tests/build, render the output, drive the CLI, script the flow). Do NOT park a task just because you'd like a playtest, and NEVER park the whole queue to wait on one check. DECIDE everything REVERSIBLE yourself and keep going — this INCLUDES design/visual/taste calls and genuinely ambiguous forks: pick the option you would RECOMMEND, note the choice and why in the task so the owner can review and reverse it on return, and move on (git + the checkpoint make it undoable). PARK — don't guess, don't execute — ONLY what you truly cannot decide because reversing it would be impossible or costly: any IRREVERSIBLE or externally-binding action (delete data, push, send, spend money), or a check you cannot run yourself (a physical device, a GUI you can't drive). Reversibility — not how design-y or how unsure you are — is the parking test. Park just that ONE item as a \"❓ [parked] <what needs deciding — with your recommendation>\" and KEEP WORKING the rest of the queue; it lands in the open-questions bucket that re-surfaces on their next prompt and shows in hud, so they review the pile when back. Do all reversible work now. This is now ENFORCED, not just asked: AskUserQuestion is hard-blocked and the Stop hook auto-continues while any non-❓ task is still queued, so parking is the ONLY way to defer to them — you cannot idle with work left. ($solo_cmd off when the owner returns.)"
