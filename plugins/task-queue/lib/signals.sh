...
tq_state_signals() {
  ...
  if tq_is_away "$root"; then
    block="🚶 AWAY mode is ON — the owner is away from the keyboard, so do NOT block on them: never call AskUserQuestion and never ask them to run a test or take a manual step. Keep going in auto and finish as much as you safely can, VERIFYING your own work YOURSELF — you have a shell, so a 'playtest' or manual check is something you RUN, not a reason to stop (run the tests/build, render the output, drive the CLI, script the flow). Do NOT park a task just because you'd like a playtest, and NEVER park the whole queue to wait on one check. PARK — don't guess, don't execute — ONLY what you genuinely cannot do without them: a design/visual/taste call, a genuinely ambiguous fork, a check you truly cannot run yourself (physical device, a GUI you can't drive), and ESPECIALLY any irreversible or externally-binding action (delete data, push, send, spend money). Park just that ONE item as a \"❓ [parked] <what needs deciding — with your recommendation>\" and KEEP WORKING the rest of the queue; it lands in the open-questions bucket that re-surfaces on their next prompt and shows in hud, so they review the pile when back. Do all reversible work now. This is now ENFORCED, not just asked: AskUserQuestion is hard-blocked and the Stop hook auto-continues while any non-❓ task is still queued, so parking is the ONLY way to defer to them — you cannot idle with work left. ($solo_cmd off when the owner returns.)"
