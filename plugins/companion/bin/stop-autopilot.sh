...
# AUTO-CONTINUE instead of stopping (keep-going mode). Self-terminates when only ❓/⏳ deferred
# items are left. A no-progress cap (consecutive stops with no task completed) yields so a
# stuck model can't spin forever. Best-effort: any error degrades to "allow the stop".
# Disable: CLAUDE_COMPANION_AUTOPILOT_CONTINUE=0; cap: CLAUDE_COMPANION_AUTOPILOT_MAX (8).
set -uo pipefail
allow() { exit 0; }
...
# Ship-mode: capture this turn's work as a reversible COMMIT on a non-default branch — never
...
if companion_ship_on "$root" && [ -n "$(git -C "$cwd" status --porcelain 2>/dev/null)" ]; then
  ( cd "$cwd" 2>/dev/null || exit 0
    ...
    # commit fails wherever git identity isn't set (CI, a fresh machine).
...
# IFS=$'\t' on the read is load-bearing: the fields are tab-joined and the subject (NEXT) can
# carry spaces — a default-IFS split would corrupt it the moment a field order changes.
IFS=$'\t' read -r OPEN DONE NEXT NID DONEWHEN < <(jq -rs '
  def pk: ((.subject//"")|sub("^[[:space:]]+";"")|(startswith("❓") or startswith("⏳")));
