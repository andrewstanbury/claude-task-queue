#!/usr/bin/env bash
# PreToolUse[AskUserQuestion] — reinstated in R100/Pass 6, owner-decided (docs/adr/README.md R105):
# "avoid rework" and "don't let the model barrel past something it should have asked about" won
# out over full portability for this one guarantee. While autopilot is ON, DENY the question.
# Autopilot means keep going without stopping (R36); asking = stopping, so the model must
# decide-if-reversible-mechanic or PARK the decision/design choice as a `❓` task instead. This
# makes autopilot's "don't stop to ask" mechanical again, not advisory. Silent (allow) when
# autopilot is off. Best-effort: degrades to allow. Claude-Code-only by construction (R104): no
# MCP-portable equivalent exists that can block an in-flight tool call, only a client-native hook.
set -uo pipefail
...
# AUTO-PARK THE INTERCEPTED QUESTION. Denying alone made the block a dead end: the guard is
# enforced, but the parking that is supposed to follow it was advisory. The hook already HAS the
# payload, so it can write a real park carrying the actual options rather than leaving a thin
# guess to be reconstructed from memory.
#
