...
#   3. Surface the repo's own LESSONS.md (accumulated gotchas) if it has one.
#   4. Prune stale per-session state.
...
# hook IS the post-compaction re-anchor. On compact it re-injects the live queue (each task's
# done-when) + LESSONS — the real cross-compaction memory — but NOT the whole STEERING (the
...
if [ "$src" = "compact" ]; then
  ...
  # Prune stale per-session state. Best-effort: silent if tidy.sh is unavailable.
  ( . "$PLUGIN_DIR/lib/tidy.sh" 2>/dev/null && tidy_prune_state 2>/dev/null ) || true
fi

carry="$(companion_open_tasks "$root")"
[ -n "$carry" ] && msg="$msg"$'\n\n'"── Open tasks carried over from an earlier session (reinstate before new work) ──"$'\n'"$carry"

# This repo's accumulated gotchas — model-maintained; first match wins.
for lf in "$root/docs/LESSONS.md" "$root/LESSONS.md" "$root/.companion/LESSONS.md"; do
  [ -f "$lf" ] || continue
  msg="$msg"$'\n\n'"── This repo's LESSONS (accumulated gotchas — heed them, and append new ones as you learn them; full file capped at 30 lines) ──"$'\n'"$(tail -n 30 "$lf")"
  break
done
