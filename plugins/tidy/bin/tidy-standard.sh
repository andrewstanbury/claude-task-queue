...
- Subtract as you add: reuse before create, delete what a change makes redundant, simplest design that fits — net complexity should trend down, not up. YAGNI — the burden of proof is on ADDING complexity: justify any new dependency/abstraction/layer by a present requirement, never a hypothetical future one (unwarranted complexity is the upstream driver of a growing blast radius). Prefer boring, reversible, replaceable choices (architecture gets no human review here, and the owner can'"'"'t recover from an irreversible or exotic one).
