...
if [ "$src" = "compact" ] || [ "$src" = "resume" ]; then
  ...
  ctx='[tidy] (standard in CLAUDE.md) clean-as-you-go, scoped to your change: blast radius first — cover the dependents of what you touch; verify changed behavior (test where it earns its keep, else types/build/run; suite green); subtract as you add — reuse before create, delete what a change makes redundant; no new seam/abstraction until something actually varies across it (deletion test: if removing a module only relocates its complexity, it was a pass-through — inline it). Keep each unit cohesive (one thing — split on "and") and shallow (early-return over deep nesting), not just short.'
else
...
- Subtract as you add: reuse before create, delete what a change makes redundant, simplest design that fits — net complexity should trend down, not up. YAGNI — the burden of proof is on ADDING complexity: justify any new dependency/abstraction/layer by a present requirement, never a hypothetical future one. Two concrete tests to apply: (a) don'"'"'t introduce a seam/abstraction until something ACTUALLY varies across it — one caller or one implementation means no seam yet (one adapter is a hypothetical seam, two is a real one); (b) the deletion test — if removing a module would erase its complexity it was a pass-through (inline it), but if the complexity reappears spread across its callers it earned its keep. (Unwarranted complexity is the upstream driver of a growing blast radius.) Prefer boring, reversible, replaceable choices (architecture gets no human review here, and the owner can'"'"'t recover from an irreversible or exotic one).
- Right altitude per unit: a function or module should do ONE thing — if naming it needs an "and", split it (cohesion is the testable form of SRP, and one reason-to-change keeps blast radius small). Prefer flat, shallow control flow — early-return over deep nesting, a short clear function over a clever dense one; branching complexity, not just line count, is what Claude and the next reader pay to load (the size nudge catches length; you own the nesting).
