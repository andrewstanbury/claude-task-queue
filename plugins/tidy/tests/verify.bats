...
# ---- coupling-density trend (watch total coupling climb) ---------------------

cd_repo() {  # $1=repo : 6 source files, 2 imports each → density 200
  mkdir -p "$1"; git -C "$1" init -q
  git -C "$1" -c user.email=t@t -c user.name=t commit -q --allow-empty -m i
  local i; for i in 1 2 3 4 5 6; do printf 'import a\nimport b\ncode\n' > "$1/f$i.js"; done
  git -C "$1" add -A; git -C "$1" -c user.email=t@t -c user.name=t commit -q -m s
}

@test "coupling density: edges-per-file centi-integer; 0 when <5 files / non-git" {
  local repo="$WORK/cd"; cd_repo "$repo"
  run bash -c '. "$1/lib/arch.sh"; tidy_coupling_density "$2"' bash "$ROOT" "$repo"
  [ "$output" = "200" ]
  run bash -c '. "$1/lib/arch.sh"; tidy_coupling_density "$2"' bash "$ROOT" "$(mktemp -d)"
  [ "$output" = "0" ]
}

@test "coupling trend: silent first (baseline), then nudges when density climbs" {
  local repo="$WORK/ct"; cd_repo "$repo"
  export CLAUDE_TIDY_TEST_CMD='true'
  printf 'x\n' >> "$repo/f1.js"                          # dirty, no new imports
  run run_verify "$repo" t1
  [ -z "$output" ]                                       # baseline established silently
  printf 'import c\nimport d\nimport e\nimport f\n' >> "$repo/f2.js"   # density spikes
  run run_verify "$repo" t2
  [[ "$output" == *"Coupling trend"* ]]
  [[ "$output" == *"imports/file"* ]]
}

@test "coupling trend: disabled via CLAUDE_TIDY_COUPLING_TREND=0" {
  local repo="$WORK/ct0"; cd_repo "$repo"
  export CLAUDE_TIDY_TEST_CMD='true' CLAUDE_TIDY_COUPLING_TREND=0
  printf 'x\n' >> "$repo/f1.js"; run run_verify "$repo" d1
  printf 'import c\nimport d\nimport e\nimport f\n' >> "$repo/f2.js"
