...
| **hud** | 0.17.0 | Show | one read-only status line — animated health beacon (orbiting dots; green ok/yellow autopilot/red tests) · always-on feature status (✈️ autopilot/🤖 agents, green on/grey off) · model · tests · floors-off · open-Qs · tokens · branch (+unpushed) (`/hud:legend` decodes every symbol) |
