...
|---|---|---|---|
| **task-queue** | 0.34.0 | Orchestrate | review loop on every prompt (steelman→challenge critique) · wireframe design preview · intent→outcome gate · open-questions tracker · cross-session resume + mid-task breadcrumb · **autopilot** (enforced autonomy — auto-continues the queue, blocks asking, decides the reversible calls, parks only the irreversible) · crash-checkpoint (working-tree snapshots) · per-feature slash commands (`/task-queue:autopilot` · `checkpoint` · `agents` · `resume` · `status`) |
...
| **hud** | 0.9.2 | Show | one read-only status line — health · always-on feature status (✈️ autopilot/🤖 agents/🧷 logs, green on/grey off) · model · tests · floors-off · open-Qs · tok · branch (+unpushed) (`/hud:legend` decodes every symbol) |
